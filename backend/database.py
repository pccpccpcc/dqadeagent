            ...
            for i in range(7):
                ...
                for result in member_results:
                    ...
                    if channel in channel_data:
                        channel_data[channel]['member_count'] = result['count']
                
                # 处理非项目组数据
                for result in non_member_results:
                    channel = result['channel'] or '未知渠道'
                    if channel in channel_data:
                        channel_data[channel]['non_member_count'] = result['count']
                
                # 添加到结果列表
                results.extend(channel_data.values())
